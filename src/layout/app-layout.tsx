...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiCalendar, FiFlag } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['party-chief', 'party-member', 'volunteer', 'event-coordinator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<JanasenaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/janasenas` : null),
    () => getJanasenas({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Janasena',
      path: isTenantUser ? `/janasenas/view/${data?.[0]?.id}` : '/janasenas',
      entity: 'janasena',
